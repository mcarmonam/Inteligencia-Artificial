...
public class Gato {
	...
	if (!this.hayGanador){
	    ...
	    for(int y = 0; y < 3; y++){  
		for(int x = 0; x < 3; x++){
		     // Checar si ya se tiro.
		    if (this.tablero[y][x] != 1 && this.tablero[y][x] != 4){
			// Creamos un gato nuevo para guardarlo en la lista de sucesores.
			Gato gatonuevo = new Gato(this);
			// Nuestro nuevo gato cambia de jugador
			gatonuevo.jugador1 = !this.jugador1;
			// Tiramos.
			gatonuevo.tiraEn(x,y);
			 // Hacemos antecesor este gato.
			gatonuevo.padre = this;
			// Checamos si el nuevo gato no está en la lista.
			if (!this.sucesores.contains(gatonuevo)){ 
			    // Lo agregamos.
			    this.sucesores.add(gatonuevo);             
			}
		    }
		}
	    }
...
// ------- *** ------- *** -------
	...
	for (int i1 = 0, j2 = 0; i1 < 3; i1++, j2++) {
	    for (int j1 = 0, i2 = 0; j1 < 3; j1++, i2++) {
		if(tablero[i1][j1] != otro.tablero[i2][j2]) {
		    return false;
		}
	    }
	}
	...
	for (int i1 = 0, j2 = 2; i1 < 3; i1++, j2--) {
	    for (int j1 = 0, i2 = 2; j1 < 3; j1++, i2--) {
		if(tablero[i1][j1] != otro.tablero[i2][j2]) {
		    return false;
		}
	    }
	...
	for (int i1 = 0, i2 = 2; i1 < 3; i1++, i2--) {
	    for (int j1 = 0, j2 = 0; j1 < 3; j1++, j2++) {
		if(tablero[i1][j1] != otro.tablero[i2][j2]) {
		    return false;
		}
	    }
