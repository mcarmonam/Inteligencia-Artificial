...
public class Termitas extends PApplet {
    ...
    class ModeloTermitas{
      ...
      */
	  ...
	  //mueve las posiciones.
          switch (rnd.nextInt(3)) {
	  case 0:
	      return (direccion + 7) % 8;
	  case 1:
	      return direccion;
	  case 2:
	      return (direccion + 1) % 8;
	  default:
	      return -1;
	  }
      }
      ...
      */
	  ...
	  int x=0;
	  int y=0;
	  //switch con las posiciones aledañas.
          switch (dir){
	  case 0:
	      x=-1;
	      y=-1;
	  ...
	  // verica la posicion apartir de donde está parado la termita. 
	  if ((t.posY+y) >= 0 && (t.posY+y) < alto && (t.posX+x) >= 0 && (t.posX+x < ancho)){
	      return mundo[t.posY+y][t.posX+x].estado;
	  }else{
	      return false;
	  }
      }
	...
	/** Simula el comportamiento de soltar una astilla y mover a la hormiga en la direccion dada.
      ...
      */
	  ...
	  mundo[t.posY][t.posX].estado = true;
	  t.cargando = false;
          //generas una nueva ubucación aleatoria.
	  int x = rnd.nextInt(ancho);
	  int y = rnd.nextInt(alto);
          //Sigue generando hasta que en la ubicación no haya astilla. 
