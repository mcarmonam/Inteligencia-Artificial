...
public class AEstrella extends PApplet {
    ...
    int nObstaculos = 60;      // Número de obstaculos aleatorios en el mapa
    Random rnd = new Random();  // Random para los números aleatorios que se necesiten
    
    // Pocision aleatoria de incio
    int inicioCol = rnd.nextInt(columnas);  
    int inicioRen = rnd.nextInt(renglones);
    // Pocision aleatoria de fin
    int finalCol = rnd.nextInt(columnas);
    int finalRen = rnd.nextInt(renglones);
    ...
    public void setup(){
        ...
        mapa.mundo[1][1].tipo = Tipo.OBSTACULO;
        mapa.mundo[1][2].tipo = Tipo.OBSTACULO;
        mapa.mundo[1][3].tipo = Tipo.OBSTACULO;
        mapa.mundo[2][3].tipo = Tipo.OBSTACULO;
        mapa.mundo[3][3].tipo = Tipo.OBSTACULO;
        mapa.mundo[4][3].tipo = Tipo.OBSTACULO;
        //mapa.mundo[6][5].tipo = Tipo.OBSTACULO;
        //mapa.mundo[4][10].tipo = Tipo.OBSTACULO;

	 //Inciamos los obstaculos aleatoriamente   
        //for (int i = 0; i < nObstaculos; i++){
	 //mapa.mundo[rnd.nextInt(renglones)][rnd.nextInt(columnas)].tipo = Tipo.OBSTACULO;
            //}
        algoritmo = new Algoritmo();
        Mosaico estadoInicial = mapa.mundo[4][1];
        Mosaico estadoFinal = mapa.mundo[3][5];
    ...
    class Mosaico{
        ...
        void calculaHeuristica(Mosaico meta){
            ...
            */
	    // simplemente sacamos la diferencia entre las columnas de ambos 
            // y las de renglones de ambos y sacamos valor absoluto por si 
            // queda negativo y simplemente lo sumamos para tener la distancia
            // manhattan.
            hn = (abs(this.renglon-meta.renglon) + abs(this.columna-meta.columna)) * 10;     
        }
    ...
    class NodoBusqueda implements Comparable<NodoBusqueda> {
		...
		// se modifica para que los sucesores solo
		// sean no visitados distintos de obstaculos
                if(sucesor != null && (sucesor.situacion == Situacion.SIN_VISITAR
                    || sucesor.situacion == Situacion.EN_LISTA_ABIERTA)
                    && sucesor.tipo != Tipo.OBSTACULO) {
    ...
    class Algoritmo {
	    ...
	    listaAbierta = new PriorityQueue<>();
	     ...
	     // Metemos al inicial a la lista abierta
            nodoPrevio = new NodoBusqueda(estadoInicial);
	    nodoPrevio.estado.situacion = Situacion.EN_LISTA_ABIERTA;
        ...
        void expandeNodoSiguiente() {
            /**
              ...
              */
	    if (!resuelto){
		// tomar primer nodo de la lista abierta
		nodoActual = listaAbierta.poll();
		nodoActual.estado.situacion = Situacion.ACTUAL;
                
		// poner al previo en la lista cerrada
		...
		if (nodoActual.estado.equals(estadoFinal)){
		    // si es, entonces ya se resolvió, cambia la situación del camino
		    // a en solución
		...
		// si no, agregamos a los sucesores que se puedan
		// a la lista abierta
		LinkedList<NodoBusqueda> sucesores = nodoActual.getSucesores();
		for (NodoBusqueda l : sucesores){
                    // si el sucesor está en la lista abierta
		    if (listaAbierta.contains(l)){
                        if (l.gn < l.estado.gn){
                            // si la ruta por la que se llega a l es de g mas 
                            // chica se actualiza a la nueva ruta
                            l.estado.padre = nodoActual.estado;
                            l.estado.gn = l.gn;
                            listaAbierta.offer(l);
                        }
                    }
                    else {
                        ...
                        l.estado.padre = nodoActual.estado;
                        l.estado.gn = l.gn;
                        l.estado.calculaHeuristica(estadoFinal);
                        l.estado.situacion = Situacion.EN_LISTA_ABIERTA;
                        listaAbierta.offer(l);
                    }
		}
